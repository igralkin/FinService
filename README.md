# FinService — REST API банковской системы на Go

**FinService** — это учебный проект, имитирующий полноценную банковскую систему. Он реализован как REST API на языке Go и включает:

- регистрацию и аутентификацию с JWT,
- банковские счета и переводы,
- виртуальные карты с шифрованием (PGP, HMAC, bcrypt),
- оформление и обслуживание кредитов,
- аналитику и прогнозирование баланса,
- автоматические email-уведомления,
- интеграцию с ЦБ РФ для получения ключевой ставки.

Цель проекта — изучить архитектуру backend-сервисов на Go, включая:

- работу с PostgreSQL и слоями репозиториев,
- бизнес-логику через сервисы,
- безопасное хранение данных,
- маршрутизацию и middleware,
- взаимодействие с внешними API (SOAP и SMTP).

Проект разделён на модули, и каждый компонент легко расширяется. Он подходит как основа для pet-проекта, курса, портфолио или обучения Go backend-разработке.
...
```bash
git clone https://github.com/igralkin/FinService.git
cd FinService
```
...
cp env.example .env
```
3. Проект использует ряд внешних библиотек. Чтобы установить их вручную, выполните:

```bash
go get github.com/golang-jwt/jwt/v5              # JWT-аутентификация
go get github.com/go-mail/mail/v2                # SMTP-отправка email-уведомлений
go get github.com/beevik/etree                   # Парсинг XML-ответов ЦБ РФ (SOAP)
go get github.com/sirupsen/logrus                # Структурированное логирование
go get golang.org/x/crypto/bcrypt                # Хеширование паролей и CVV
```
После этого можно запустить:
```bash
go mod tidy
```
4. Запустите скрипт `db.sql` (находится в корне проекта) или выполните команды для установки расширения `pgcrypto` и создания нужных таблиц в базе данных Postgres, или, например, следующей командой в командной строке:
```bash
psql -U postgres -d fin_service -f db.sql
```
5. Запустите сервер:
```bash
go run main.go
...
Проект использует библиотеку `logrus` для структурированного логирования (с таймстемпами, уровнями: info, warn, error). Все системные и бизнес-события логируются в stdout.

## Работа с БД

В проекте используются параметризованные SQL-запросы во всех репозиториях:

```go
err := db.QueryRow("SELECT id FROM users WHERE email = $1", email).Scan(&id)
```
Это предотвращает SQL-инъекции. Все INSERT/UPDATE/DELETE/SELECT оформлены через подготовленные выражения и работу с database/sql.
...
Все ошибки БД перехватываются и транслируются в пользовательские сообщения. Например:

- при повторной регистрации с тем же email возвращается:
```json
{
  "error": "email already in use"
}
```
- при недопустимой операции с чужим счётом — `403 Forbidden`
В коде реализована интерпретация ошибок PostgreSQL по коду ошибки (например, unique_violation).
...
```
Функционал:
- Генерируется валидный номер карты (с использованием алгоритма Луна)
- Срок действия: 3 года вперёд от текущей даты
- CVV: случайный 3-значный код
- Данные шифруются и хранятся в БД:
  - Номер карты, месяц и год — AES-256 GCM
  - CVV — bcrypt
  - HMAC-сигнатура — контроль целостности по HMAC-SHA256
